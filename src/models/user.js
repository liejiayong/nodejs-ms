...
exports.findDataByName = (name) => {
    let _sql = `select * from jy_blog_users where username="${name}";`;
    return query(_sql);
};
...
exports.loginUser = (value) => {
    let _sql = `select id, username, nickname, email, phone from jy_blog_users where username=? and password=?;`;
    return query(_sql, value);
};
